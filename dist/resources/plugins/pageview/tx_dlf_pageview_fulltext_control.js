 ...
 */
var dlfViewerFullTextControl = function dlfViewerFullTextControl(map, image, fulltextUrl) {
     ...
     * @private
     */
    this.dic = $('#tx-dlf-tools-fulltext').length > 0 && $('#tx-dlf-tools-fulltext').attr('data-dic') ? dlfUtils.parseDataDic($('#tx-dlf-tools-fulltext')) : { 'fulltext-on': 'Activate Fulltext', 'fulltext-off': 'Dectivate Fulltext' };
    ...
    this.handlers_ = {
        mapClick: $.proxy(function (event) {
            ...
            var feature = this.map.forEachFeatureAtPixel(event['pixel'], function (feature, layer) {
                if (feature.get('type') === 'textblock') return feature;
            });
            ...
            if (dlfUtils.exists(feature)) this.showFulltext([feature]);
        }, this),
        mapHover: $.proxy(function (event) {
            ...
            map_.forEachFeatureAtPixel(event['pixel'], function (feature, layer) {
                if (feature.get('type') === 'textblock') textblockFeature = feature;
                if (feature.get('type') === 'textline') textlineFeature = feature;
            });

            //
            // Handle TextBlock elements
            //
            var activeSelectTextBlockEl_ = selectSource_.getFeatures().length > 0 ? selectSource_.getFeatures()[0] : undefined,
                activeHoverTextBlockEl_ = hoverSourceTextblock_.getFeatures().length > 0 ? hoverSourceTextblock_.getFeatures()[0] : undefined,
                isFeatureEqualSelectFeature_ = activeSelectTextBlockEl_ !== undefined && textblockFeature !== undefined && activeSelectTextBlockEl_.getId() === textblockFeature.getId() ? true : false,
                isFeatureEqualToOldHoverFeature_ = activeHoverTextBlockEl_ !== undefined && textblockFeature !== undefined && activeHoverTextBlockEl_.getId() === textblockFeature.getId() ? true : false;
            ...
            //
            var activeHoverTextBlockEl_ = hoverSourceTextline_.getFeatures().length > 0 ? hoverSourceTextline_.getFeatures()[0] : undefined,
                isFeatureEqualToOldHoverFeature_ = activeHoverTextBlockEl_ !== undefined && textlineFeature !== undefined && activeHoverTextBlockEl_.getId() === textlineFeature.getId() ? true : false;
    ...
    $("#tx-dlf-tools-fulltext").text(this.dic['fulltext-on']).attr('title', this.dic['fulltext-on']);
...
dlfViewerFullTextControl.prototype.disableFulltextSelect = function () {
    ...
    $("#tx-dlf-tools-fulltext").removeClass(className).text(this.dic['fulltext-on']).attr('title', this.dic['fulltext-on']);
...
dlfViewerFullTextControl.prototype.enableFulltextSelect = function (textBlockFeatures, textLineFeatures) {
    ...
    var className = 'fulltext-visible';
    $("#tx-dlf-tools-fulltext").addClass(className).text(this.dic['fulltext-off']).attr('title', this.dic['fulltext-off']);
...
dlfViewerFullTextControl.fetchFulltextDataFromServer = function (url, image, opt_offset) {
    ...
    var offset = dlfUtils.exists(opt_offset) ? opt_offset : undefined,
        ...
        fulltextCoordinates = request.responseXML ? parser.parseFeatures(request.responseXML) : request.responseText ? parser.parseFeatures(request.responseText) : [];
...
dlfViewerFullTextControl.prototype.showFulltext = function (features) {
     ...
     * @return {string}
     */
    appendHTML = function appendHTML(feature) {
        var html = '',
        ...
        for (var i = 0; i < textlines.length; i++) {
            ...
            for (var j = 0; j < content.length; j++) {
                html = html + '<span class="' + content[j].get('type') + '" id="' + content[j].getId() + '">' + content[j].get('fulltext').replace(/\n/g, '<br />') + '</span>';
            }
...
//# sourceMappingURL=tx_dlf_pageview_fulltext_control.js.map
