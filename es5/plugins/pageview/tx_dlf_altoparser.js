 ...
 */
var dlfAltoParser = function dlfAltoParser(opt_imageObj, opt_width, opt_height, opt_offset) {
...
dlfAltoParser.prototype.parseFeatures = function (document) {
    ...
    for (var i = 0; i < pageElements.length; i++) {
        ...
        feature.getTextblocks = function () {
            if (this.get('printspace') !== undefined && this.get('printspace').get('textblocks')) return this.get('printspace').get('textblocks');
            return [];
        };
...
dlfAltoParser.prototype.parseGeometry_ = function (node) {
    ...
    if (!dlfUtils.exists(this.image_) || !dlfUtils.exists(this.width_)) return new ol.geom.Polygon(coordinatesWithoutScale);
    ...
    for (var i = 0; i < coordinatesWithoutScale[0].length; i++) {
        coordinatesRescale.push([offset + scale * coordinatesWithoutScale[0][i][0], displayedImageHeight - scale * coordinatesWithoutScale[0][i][1]]);
    };
...
//# sourceMappingURL=tx_dlf_altoparser.js.map
