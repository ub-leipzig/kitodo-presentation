...
dlfViewerSource.tileLoadFunction = function (tileSize, tile, url) {
    ...
    $(img).load(function () {
        if (img.naturalWidth > 0 && (img.naturalWidth != tileSize || img.naturalHeight != tileSize)) {
            ...
            var key = dlfViewerSource.findKey(tile, function (v) {
                return v == img;
            });
...
dlfViewerSource.IIIF = function (options) {
    ...
    var maxZoom = Math.max(Math.ceil(Math.log(width / tileSize) / Math.LN2), Math.ceil(Math.log(height / tileSize) / Math.LN2));
    ...
    var tileUrlFunction = function tileUrlFunction(tileCoord, pixelRatio, projection) {
        ...
        if (x < 0 || sizes[0] <= x || y < 0 || sizes[1] <= y) {
            ...
            var query = '/' + minx + ',' + miny + ',' + (maxx - minx) + ',' + (maxy - miny) + '/pct:' + 100 / scale + '/0/' + quality + '.' + format;
...
dlfViewerSource.IIIF.getMetdadataURL = function (baseUrl) {
    var pathString = baseUrl.lastIndexOf('/') + 1 === baseUrl.length ? 'info.json' : '/info.json';
...
dlfViewerSource.IIP = function (options) {
    ...
    // parse parameters
    var url = options.url.indexOf('?') > -1 ? options.url : options.url + '?',
        width = options.size[0],
        height = options.size[1],
        tileSize = options.tileSize !== undefined ? options.tileSize : 256,
        origin = options.crossOrigin !== undefined ? options.crossOrigin : '*',
...
dlfViewerSource.IIP.getMetdadataURL = function (baseUrl) {
    var queryString = '&obj=IIP,1.0&obj=Max-size&obj=Tile-size&obj=Resolution-number',
        url = baseUrl.indexOf('?') > -1 ? baseUrl : baseUrl + '?';
...
//# sourceMappingURL=tx_dlf_ol3_source.js.map
