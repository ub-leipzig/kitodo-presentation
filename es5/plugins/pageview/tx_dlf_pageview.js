 ...
 * @constructor
 */
var dlfViewer = function dlfViewer(settings) {
...
dlfViewer.prototype.addHighlightField = function (highlightField, imageIndex, width, height) {
    ...
    if (this.map) this.displayHighlightWord();
};
...
dlfViewer.prototype.displayHighlightWord = function () {
    ...
    if (this.highlightFields.length > 0) {
        ...
        for (var i = 0; i < this.highlightFields.length; i++) {
            ...
            var field = this.highlightFields[i],
                coordinates = [[[field[0], field[1]], [field[2], field[1]], [field[2], field[3]], [field[0], field[3]], [field[0], field[1]]]],
                offset = this.highlightFieldParams.index === 1 ? this.images[0].width : 0;
            var feature = dlfUtils.scaleToImageSize([new ol.Feature(new ol.geom.Polygon(coordinates))], this.images[this.highlightFieldParams.index], this.highlightFieldParams.width, this.highlightFieldParams.height, offset);
    ...
    if (urlParams != undefined && urlParams.hasOwnProperty(key) && this.fulltexts[0] !== undefined && this.fulltexts[0].url !== '' && this.images.length > 0) {
        ...
        var stringFeatures = fulltextDataImageTwo === undefined ? fulltextData.getStringFeatures() : fulltextData.getStringFeatures().concat(fulltextDataImageTwo.getStringFeatures());
...
dlfViewer.prototype.init = function (controlNames) {
    ...
    if (this.imageUrls.length <= 0) throw new Error('Missing image source objects.');
    ...
    this.initLayer(this.imageUrls, this.isCorsEnabled).done($.proxy(function (layers) {

        var controls = controlNames.length > 0 || controlNames[0] === "" ? this.createControls_(controlNames, layers) : [];
...
dlfViewer.prototype.initLayer = function (imageSourceObjs, isCorsEnabled) {
    ...
    dlfUtils.fetchImageData(imageSourceObjs).done(function (imageSourceData) {
        resolveCallback(imageSourceData, dlfUtils.createOl3Layers(imageSourceData, origin));
    });

    return deferredResponse;
};
//# sourceMappingURL=tx_dlf_pageview.js.map
